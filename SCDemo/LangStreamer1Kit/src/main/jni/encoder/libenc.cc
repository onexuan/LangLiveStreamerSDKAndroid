...
static struct YuvFrame *i420_no_scaled_frame;

static bool convert_to_i420(jbyte *src_frame, jint src_width, jint src_height, jint stride,
                            jboolean need_flip, jint rotate_degree, int format) {
    i420_no_scaled_frame = NULL;
    if (stride == 0) stride = src_width;
    ...
    //LIBENC_LOGE("ConvertToI420 ry %p ru %p rv %p src_frame %p size %d stride %d src_width %d src_height %d format %#x", i420_scaled_frame.y, i420_rotated_frame.u, i420_rotated_frame.v, src_frame, y_size, stride, src_width, src_height, format);
    jint ret = ConvertToI420((uint8_t *) src_frame, stride,
                             i420_rotated_frame.y, src_width,
                             i420_rotated_frame.u, src_width / 2,
                             i420_rotated_frame.v, src_width / 2,
                             0, 0,
                             stride, src_height,
    ...
    if (i420_rotated_frame.width == i420_scaled_frame.width &&
            i420_rotated_frame.height == i420_scaled_frame.height && !need_flip) {
        i420_no_scaled_frame = &i420_rotated_frame;
    }else {
         LIBENC_LOGE("Using I420Scale rw %d rh %d sw %d sh %d", i420_rotated_frame.width, i420_rotated_frame.height, i420_scaled_frame.width, i420_scaled_frame.height);
        ret = I420Scale(i420_rotated_frame.y, i420_rotated_frame.width,
                ...
                kFilterNone);
            i420_no_scaled_frame = &i420_scaled_frame;
    }
    if (ret < 0) {
         LIBENC_LOGE("I420Scale failure rw %d rh %d sw %d sh %d", i420_rotated_frame.width, i420_rotated_frame.height, i420_scaled_frame.width, i420_scaled_frame.height);
         return false;
    }
...
static jbyteArray libenc_RGBAToI420(JNIEnv* env, jobject thiz, jbyteArray frame, jint src_width,
    ...
    if (!convert_to_i420(rgba_frame, src_width, src_height,0, need_flip, rotate_degree, FOURCC_RGBA)) {
        return NULL;
    }

    int y_size = i420_no_scaled_frame->width * i420_no_scaled_frame->height;
    jbyteArray i420Frame = env->NewByteArray(y_size * 3 / 2);
    env->SetByteArrayRegion(i420Frame, 0, y_size * 3 / 2, (jbyte *) i420_no_scaled_frame->data);
...
static jbyteArray libenc_convertToI420(JNIEnv* env, jobject thiz, jbyteArray frame,jlong addr, jint src_width,
                                    jint src_height,jint stride, jboolean need_flip, jint rotate_degree, int srctype) {
    jbyte* rgba_frame = NULL;
    if (addr != 0)
        rgba_frame = (jbyte*) addr;
    else
         rgba_frame = env->GetByteArrayElements(frame, NULL);

    if (!convert_to_i420(rgba_frame, src_width, src_height, stride, need_flip, rotate_degree, srctype)) {
        return NULL;
    }
...
static void libenc_RGBAToI420Fast(JNIEnv* env, jobject thiz, jbyteArray srcframe, jlong addr, jbyteArray dstframe, jint src_width,
                                    jint src_height,jint src_stride, jboolean need_flip, jint rotate_degree, int srctype) {

    jbyte* rgba_frame = NULL;
    if (addr != 0)
        rgba_frame = (jbyte*) addr;
    else
        rgba_frame = env->GetByteArrayElements(srcframe, NULL);
	...
	jint ret = ConvertToI420((uint8_t *) rgba_frame, src_stride,
                             ...
                             src_width, src_height,
                             (RotationMode) rotate_degree, srctype);

    if (ret < 0) {
        LIBENC_LOGE("libenc_RGBAToI420Fast failure");
    }

	if (srcframe != NULL)
		env->ReleaseByteArrayElements(srcframe, rgba_frame, JNI_ABORT);
	env->ReleaseByteArrayElements(dstframe, yuv_frame, JNI_ABORT);
}

static void libenc_I420ToNV12Fast(JNIEnv* env, jobject thiz, jbyteArray i420frame, jbyteArray nv12frame, jint src_width, jint src_height) {
...
static jbyteArray libenc_RGBAToNV12(JNIEnv* env, jobject thiz, jbyteArray frame, jint src_width,
    ...
    int ret = ConvertFromI420(i420_no_scaled_frame->y, i420_no_scaled_frame->width,
                              i420_no_scaled_frame->u, i420_no_scaled_frame->width / 2,
                              i420_no_scaled_frame->v, i420_no_scaled_frame->width / 2,
...
static jbyteArray libenc_convertToNV12(JNIEnv* env, jobject thiz, jbyteArray frame,jlong addr, jint src_width,
                                    jint src_height,jint stride, jboolean need_flip, jint rotate_degree, jint src_type) {
    jbyte* rgba_frame = NULL;
    if (addr != 0)
        rgba_frame = (jbyte* )addr;
    else
    ...
    if (!convert_to_i420(rgba_frame, src_width, src_height, stride, need_flip, rotate_degree, src_type)) {
        return NULL;
    }
...
static jint libenc_RGBASoftEncode(JNIEnv* env, jobject thiz, jbyteArray frame, jint src_width,
    ...
    int es_len = x264_ctx.global_nal_header ? encode_global_nal_header() : x264_encode(i420_no_scaled_frame, pts);
    if (es_len <= 0) {
...
static jint libenc_softEncode(JNIEnv* env, jobject thiz, jbyteArray frame,jint src_width,
                                  jint src_height, jboolean need_flip, jint rotate_degree, jlong pts, jint type) {


    struct YuvFrame *yuvframe = NULL;
    struct YuvFrame yuv_from_addr;
    jbyte* raw_frame = env->GetByteArrayElements(frame, NULL);
    if (type != FOURCC_I420) {
            
            if (!convert_to_i420(raw_frame, src_width, src_height,0, need_flip, rotate_degree, type)) {
                return JNI_ERR;
            }
            yuvframe = i420_no_scaled_frame;
    }else {
     yuv_from_addr.width = src_width;
     yuv_from_addr.height = src_height;
     yuv_from_addr.data = (uint8_t *)raw_frame;
     yuv_from_addr.y = yuv_from_addr.data ;
     yuv_from_addr.u = yuv_from_addr.y + src_width * src_height;
     yuv_from_addr.v = yuv_from_addr.u + src_width * src_height / 4;
     yuvframe = &yuv_from_addr;
    }

    int es_len = x264_ctx.global_nal_header ? encode_global_nal_header() : x264_encode(yuvframe, pts);
    if (es_len <= 0) {
        LIBENC_LOGE("Fail to encode nalu");
        return JNI_ERR;
    }
    ...
    env->ReleaseByteArrayElements(frame, raw_frame, JNI_ABORT);
    return JNI_OK;
}
...
static void libenc_closeSoftEncoder(JNIEnv* env, jobject thiz) {
    ...
    if (x264_ctx.encoder != NULL) {
        ...
        memset(&x264_ctx, 0, sizeof(x264_ctx));
    }
}
...
static jboolean libenc_openSoftEncoder(JNIEnv* env, jobject thiz) {
    ...
    if (x264_param_apply_profile(&x264_ctx.params, "main" ) < 0) {
...
static JNINativeMethod libenc_methods[] = {
    ...
    { "RGBAToI420Fast", "([BJ[BIIIZII)V", (void *)libenc_RGBAToI420Fast },
    { "I420ToNV12Fast", "([B[BII)V", (void *)libenc_I420ToNV12Fast },
    { "ConvertToI420", "([BJIIIZII)[B", (void *)libenc_convertToI420 },
    { "RGBAToNV12", "([BIIZI)[B", (void *)libenc_RGBAToNV12 },
    { "ConvertToNV12", "([BJIIIZII)[B", (void *)libenc_convertToNV12 },
    ...
    { "softEncode", "([BIIZIJI)I", (void *)libenc_softEncode },
};
...
jint JNI_OnLoad(JavaVM* vm, void* reserved) {
    ...
    const char * classstr = "net/lang/streamer/engine/LangVideoEncoderImpl";
    jclass clz = jenv->FindClass(classstr);//jenv->FindClass("net/ossrs/yasea/SrsEncoder");
    if (clz == NULL) {
        LIBENC_LOGE("Class %s not found", classstr);
        return JNI_ERR;
    }
