...
public class SenseMEFilter extends GPUImageFilter {
    ...
    private static final String MODEL_NAME_FACE_EXTRA = "M_SenseME_Face_Extra_5.23.0.model";
...
//            deinitSticker();
    ...
    public void setDetectionHandler(LangFaceHandler handler) {
        mHandler = handler;
    }
    ...
    public void setFaceuConfig(SenseMEFilter.FaceuConfig faceuConfig) {
        synchronized (mLock) {
...
//                    | STMobileHumanActionNative.ST_MOBILE_DETECT_EXTRA_FACE_POINTS
    ...
    //---------------------------------------------------------------------
    private LangFaceHandler.GestureType getGesture(STHumanAction humanAction) {
        if (humanAction.handCount > 0) {
            long handAction = humanAction.hands[0].handAction;
