...
#ifndef I_GRAPHIC_BUFFER_H
#define I_GRAPHIC_BUFFER_H
...
typedef struct IGrapicBufferWrapper {
    graphics_handle*    (*alloc)(int width, int height, graphics_pixformat pixfmt, int usage);
    void        (*destroy)(graphics_handle *);
    ...
    int32_t     (*pixelFormat)(graphics_handle *);
    ARect       (*rect)(graphics_handle *);
    graphics_native_handle  (*handle)(graphics_handle *, int *size);
    void*       (*winbuffer)(graphics_handle *);
    int32_t     (*reallocate)(graphics_handle *, uint32_t w, uint32_t h, graphics_pixformat f, uint32_t usage);
    int32_t     (*lock)(graphics_handle *, uint32_t usage, void** vaddr);
    int32_t     (*lockRect)(graphics_handle *, uint32_t usage, const ARect& rect, void** vaddr);
    int32_t     (*unlock)(graphics_handle *);
    int32_t     (*lockYCbCr)(graphics_handle *, uint32_t usage, graphics_ycbcr *ycbcr);
    int32_t     (*lockYCbCrRect)(graphics_handle *, uint32_t usage, const ARect& rect, graphics_ycbcr *ycbcr);
} IGrapicBufferWrapper;

#ifdef __cplusplus
}
#endif
#endif //I_GRAPHIC_BUFFER_H
